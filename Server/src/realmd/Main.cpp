...
char serviceLongName[] = "Login Server Classic";
...
extern int main(int argc, char** argv)
{
    ...
    sLog.outString("%s [Login Server]", REVISION_NR);
    sLog.outString("<STRG-C> zum Beenden\n");
    ...
    while (!stopEvent)
    {
        ...
        {
            loopCounter = 0;
            DETAIL_LOG("Rufe den MySQL Server um die Verbindung aufrecht zu halten");
            LoginDatabase.Ping();
        }
#ifdef WIN32
...
bool StartDB()
{
    ...
    sLog.outString("Login Datenbank Verbindungen: %i", 1 + 1);
